    ...
    const fetchUserInfo = async () => {
      ...
      if (token) {
        ...
        if (user) {
          ...
          console.log('[Theme] AppContextProvider: sessionStorage=', sessionStorage.getItem('globy_theme'), 'url=', new URLSearchParams(window.location.search).get('theme'), 'resolved=', storedTheme);
          try {
            const verifyUserRes = await verifyUser(token, payload || {}, storedTheme);
            console.log('[Theme] verifyUser response:', verifyUserRes?.data);
